...
By Exercise 5, if $X$ deformation retracts to $(r,s)$ then $V \hookrightarrow U$ 
is nullhomotopic. However, this requires $V$ to be path-connected since the map 
$t \mapsto f_t(x)$ yields a path connecting $x$ to $x_0 = f_1(x)$. On the other 
hand, the neighborhood $V$ we just picked is clearly not path-connected.
